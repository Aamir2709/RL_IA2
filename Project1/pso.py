...
def optimize_prices():
    ...
    data = pd.read_csv("pso_data.csv")
    opt_data = pd.read_sql_table("sample_data_updated")
    ...
    for index, row in optimization_results.iterrows():
        arxikiTimi = opt_data.loc[opt_data["product_id"] == row["product_id"]].initial_price.values
        arxikiTimi = round(float(arxikiTimi), 2)
        telikiTimi = opt_data.loc[opt_data["product_id"] == row["product_id"]].final_price.values
